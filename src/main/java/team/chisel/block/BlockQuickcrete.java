...
public class BlockQuickcrete extends BlockCarvable {
	float speedup=0;
	public BlockQuickcrete() {
		super();
		...
		if (event.phase == Phase.START && event.side.isClient() && event.player.onGround && event.player instanceof EntityPlayerSP) {
			...
			if (below == this) {
				...
				if (manualInputCheck.moveForward != 0 || manualInputCheck.moveStrafe != 0) {
					player.motionX *= Configurations.concreteVelocityMult + speedup;
					player.motionZ *= Configurations.concreteVelocityMult + speedup;
				}
			}
		}
	}
	public Block setSpeedup(float value){
		speedup=value;
		return this;
	}
}
