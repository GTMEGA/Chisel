...
public class RenderCarvableBeacon extends TileEntityBeaconRenderer implements ISimpleBlockRenderingHandler, ThreadSafeBlockRenderer {
	...
	public void renderTileEntityAt(TileEntityCarvableBeacon beacon, double x, double y, double z, float partialTicks) {
		...
		if (f1 > 0.1F) {
			renderBeam(f1, beacon.getWorldObj(), x, y, z, beacon.getBlockMetadata(), partialTicks);
		}
	}
	...
	private void renderBeam(float f1, World world, double x, double y, double z, int meta, float partialTicks) {
		...
		float f2 = world.getTotalWorldTime() + partialTicks;
	...
	public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
		...
		renderBeam(0.00195f, Minecraft.getMinecraft().theWorld, 0, 0, 0, metadata, 0);
		GL11.glPopMatrix();
	}
